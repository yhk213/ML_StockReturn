# For portfolio construdction of Gu Kelly Xiu
# 
library(dplyr)
library(readr)

y_true <- read_csv('y_true.csv')
y_ela <- read_csv('y_elastic.csv')
size <- read_csv('size.csv',)

y_ela[,4] = y_true$excess_return

tidy1 <- left_join(y_ela, size, by  = c("permno" = 'permno', 'DATE' = 'DATE') )

tidy2 <-  tidy1 %>% rename(., y_real = ...4) %>%
    select(-X1)
tidy2

remove(size,tidy1,y_ela,y_true)

## 데이터 변환 끝 

tidy3<- tidy2 %>% 
    group_by(DATE) %>% mutate( Rank = ntile(elastic,10))
tidy4 <- tidy3 %>% ungroup()
# 1이 낮은거 ~ 10이 제일 높은 거 
...
port_tb1 <- port_tb %>% 
    ...
    mutate( CS1 = cumsum(log_ret_P1), 
            ...
            RET10 = exp(CS10) -1 )
library(ggplot2)
ggplot( data = port_tb1E) +
    geom_line(mapping = aes( x = Date , y = RET1) , size = 1 ) + 
    geom_line(mapping = aes( x = Date , y = RET2) , size = 1, color = "orange" ) + 
    geom_line(mapping = aes( x = Date , y = RET3) , size = 1, color = 'red' ) + 
    geom_line(mapping = aes( x = Date , y = RET4) , size = 1, color = 'blue' ) +
    geom_line(mapping = aes( x = Date , y = RET5) , size = 1, color = 'magenta' ) +
    geom_line(mapping = aes( x = Date , y = RET6) , size = 1, color = 'green' ) +
    geom_line(mapping = aes( x = Date, y = RET7) , size = 1, color = 'magenta' ) +
    geom_line(mapping = aes( x = Date, y = RET8) , size = 1, color = 'magenta' , alpha = 0.3 ) + 
    geom_line(mapping = aes( x = Date , y = RET9) , size = 1 , color = 'blue', alpha = 0.5) +
    geom_line(mapping = aes( x = Date, y = RET10) , size = 1 , color = 'steelblue') 

write.csv(port_tb1,'enet.csv')
write.csv(port_tb1E,'enet_equal.csv')
