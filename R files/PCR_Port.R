# For portfolio construction of Gu Kelly Xiu
...
y_pca <- read_csv('y_pca.csv')
size <- read_csv('size.csv',)

y_pca[,4] = y_true$excess_return

tidy1 <- left_join(y_pca, size, by = c("permno" = 'permno', 'DATE' = 'DATE') )

tidy2 <-  tidy1 %>% rename(., y_real = ...4) %>%
    select(-X1)

remove(size,tidy1,y_pca,y_true)

tidy3<- tidy2 %>% 
    group_by(DATE) %>% mutate( Rank = ntile(PCR,10))
## 데이터 전처리 완료 
...
port_tb <- port_tb %>% mutate(P1 = P1, P2 = P2 , P3 = P3, P4 = P4,
           P5 = P5, P6= P6, P7 = P7, P8 = P8, P9 = P9, P10 = P10)
port_tb1 <- port_tb %>% 
    mutate( log_ret_P1 = log(1 + P1),log_ret_P2 = log(1 + P2),
            log_ret_P3 = log(1 + P3),log_ret_P4 = log(1 + P4),
            log_ret_P5 = log(1 + P5),log_ret_P6 = log(1 + P6),
            log_ret_P7 = log(1 + P7),log_ret_P8 = log(1 + P8),
            log_ret_P9 = log(1 + P9),log_ret_P10 = log(1 + P10),
    ) %>% 
    mutate( CS1 = cumsum(log_ret_P1), CS2 = cumsum(log_ret_P2),
            CS3 = cumsum(log_ret_P3), CS4 = cumsum(log_ret_P4),
            CS5 = cumsum(log_ret_P5), CS6 = cumsum(log_ret_P6),
            CS7 = cumsum(log_ret_P7), CS8 = cumsum(log_ret_P8),
            CS9 = cumsum(log_ret_P9), CS10 = cumsum(log_ret_P10),
            RET1 = exp(CS1) -1, RET2 = exp(CS2) -1,
            RET3 = exp(CS3) -1, RET4 = exp(CS4) -1,
            RET5 = exp(CS5) -1, RET6 = exp(CS6) -1,
            RET7 = exp(CS7) -1, RET8 = exp(CS8) -1,
            RET9 = exp(CS9) -1, RET10 = exp(CS10) -1 )
...
write_csv(port_tb1,'pcr.csv')
write.csv(port_tb1E,'pcr_equal.csv')
