...
y_rf <- read_csv('y_rf.csv')

size <- read_csv('size.csv')

y_rf[,4] = y_true$excess_return

tidy1 <- left_join(y_rf, size, by  = c("permno" = 'permno', 'DATE' = 'DATE') )

tidy2 <-  tidy1 %>% rename(., y_real = ...4) %>%
    select(-X1)
tidy2

remove(size,tidy1,y_rf,y_true)

## 데이터 변환 끝 

tidy3<- tidy2 %>% 
    group_by(DATE) %>% mutate( Rank = ntile(RF,10))
tidy4 <- tidy3 %>% ungroup()
# 1이 낮은거 ~ 10이 제일 높은 거 
...
write.csv(port_tb1,'rf.csv')
write.csv(port_tb1E,'rf_equal.csv')
