...
y_pls <- read_csv('y_pls.csv')
size <- read_csv('size.csv',)

y_pls[,4] = y_true$excess_return

tidy1 <- left_join(y_pls, size, by  = c("permno" = 'permno', 'DATE' = 'DATE') )

tidy2 <-  tidy1 %>% rename(., y_real = ...4) %>%
    select(-X1)
tidy2

remove(size,tidy1,y_pls,y_true)

## 데이터 변환 끝 

tidy3<- tidy2 %>% 
    group_by(DATE) %>% mutate( Rank = ntile(PLS,10))
tidy4 <- tidy3 %>% ungroup()
# 1이 낮은거 ~ 10이 제일 높은 거 
...
write_csv(port_tb1,'pls.csv')
write_csv(port_tb1E,'pls_equal.csv')
